...
# MAGIC %md
# MAGIC
# MAGIC # 06 Gold — Events Dimension
# MAGIC
# MAGIC **Notebook:** `06_dm_eventos`
# MAGIC
# MAGIC Builds the curated Gold events dimension used by analytical models and business marts.
# MAGIC
# MAGIC This notebook defines:
# MAGIC
# MAGIC * Legislative event dimensional model
# MAGIC * Legislative event surrogate key generation
# MAGIC * Legislative event descriptive attributes
# MAGIC * Event classification attributes
...
# MAGIC
# MAGIC * Read validated legislative event records from Silver
# MAGIC * Keep one analytical record per legislative event
# MAGIC * Create the legislative event surrogate key
...
# MAGIC * Publish the Gold events dimension
# MAGIC
# MAGIC ---
# MAGIC
...
# MAGIC ### Source
# MAGIC
# MAGIC `brazil_legislative_analytics.silver.slv_eventos`
# MAGIC
# MAGIC ### Target
# MAGIC
...
NOTEBOOK_NAME = "06_dm_eventos"

ENTITY_NAME = "eventos"

SOURCE_TABLE = f"{SILVER_SCHEMA}.slv_eventos"

TARGET_TABLE = f"{GOLD_SCHEMA}.dm_eventos"
...
df_gold = (
    df_silver
    .filter(
        F.col("evt_fl_registro_valido_silver") == True
    )
)
    ...
    .withColumn(
        ...
        F.sha2(
            F.concat_ws(
                "||",
                F.col("evt_id_evento").cast("string"),
                F.col("evt_tx_titulo").cast("string"),
                F.col("evt_dt_inicio").cast("string"),
                F.col("evt_tx_tipo_evento").cast("string"),
                F.col("evt_tx_sigla_orgao").cast("string")
            ),
            256
        )
    )
...
TABLE_COMMENT = """
Gold legislative events dimension.

This dimension contains one record per legislative event.
...
COLUMN_COMMENTS = {
    "evt_sk_evento":
        "Gold surrogate key for legislative event dimension.",

    "evt_id_evento":
        "Business identifier of the legislative event from the source system.",
...
write_pipeline_log(
    ...
    target_table=TARGET_TABLE,
    status="SUCCESS",
    message="Gold legislative events dimension generated successfully.",
...
print("=" * 80)
print("DIMENSÃO EVENTOS - RESUMO EXECUÇÃO")
print("=" * 80)
